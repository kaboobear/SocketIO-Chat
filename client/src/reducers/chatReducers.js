...
export default function (state = initialState, action) {
    switch (action.type) {
        ...
        case CHATS_LOADED:
            return {
                ...state,
                chats: (action.payload.length !== 0) ? action.payload.slice().sort((a, b) => (new Date(b.lastMessageDate) - new Date(a.lastMessageDate))) : action.payload,
                chatsLoading: false
            }
        case FOCUS_MESSAGE:
            let exists = false;
        ...
        case DELETE_MESSAGE:
            return {
                ...state,
                messages: state.messages.filter(elem => !action.payload.includes(elem._id)),
                focused:[]
            }
        case CHANGE_CHAT:
            return {
        ...
        case UPDATE_CHAT:
            return {
                ...
                ).slice().sort((a, b) => (new Date(b.lastMessageDate) - new Date(a.lastMessageDate))),
            }
