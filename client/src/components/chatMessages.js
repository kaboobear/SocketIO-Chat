            ...
            <Scrollbars ref="scrollbars" className="chat-messages">
                {(chatsIsLoading)
                    ...
                    : ((messages.length === 0)
                        ...
                        : (messages.map(elem => (
                            <div key={elem._id} className={`message-item ${(elem.author._id !== user._id) ? 'friend ' : ''} ${(this.props.focused.includes(elem._id)) ? 'focused ' : ''}`} onClick={()=>{this.focus(elem._id)}}>
...
export default withRouter(connect(mapStateToProps, { getUsers,focusMessage })(ChatMessages));