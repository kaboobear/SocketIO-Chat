...
class Sidebar extends React.Component {
  ...
  logoutFun() {
    this.props.logout();
    this.props.socket.emit('UserIsLoggedOut', {
      user: this.props.user,
      current_chat: this.props.currentChat,
    });
  }
  ...
  isFriend(user_id) {
    ...
    this.props.chats.forEach((elem, id) => {
      const friend_id =
        elem.sender._id === this.props.user._id
          ? elem.talker._id
          : elem.sender._id;
      ...
      <div className="chat-sidebar">
        <ul className="user-block">
          {isLoading === false && !isAuth ? (
            <span>
              <li>
                <NavLink exact className="btn simple" to="/login">
                  Login
                </NavLink>
              </li>
              <li>
                <NavLink exact className="btn simple" to="/register">
                  Register
                </NavLink>
              </li>
            </span>
            ...
            <span>
              <li>
                <h3 className="user-title">{isAuth && user.username}</h3>
              </li>
        ...
        <Tabs>
          <TabList className="tab-buttons">
            <Tab className="tab-button" selectedClassName="tab-button-active">
              Friends
            </Tab>
            <Tab className="tab-button" selectedClassName="tab-button-active">
              World
                ...
                chats.map((elem) => (
                  <div
                    ...
                    className={`friend-item ${
                      currentChat !== 0 && elem._id === currentChat._id
                        ? 'active'
                        : ''
                    }`}
                    onClick={() => {
                  ...
                  >
                    <div className="friend-name">
                      {elem.sender._id === user._id
                        ? elem.talker.username
                        : elem.sender.username}
                    </div>

                    {elem.sender._id === user._id
                      ? elem.talker.isOnline && (
                          <div className="online-mark"></div>
                        )
                      : elem.sender.isOnline && (
                          <div className="online-mark"></div>
                        )}

                    {elem.sender._id === user._id
                      ? elem.unreadTalkerMessages > 0 && (
                          <div className="unread-count">
                            {elem.unreadTalkerMessages}
                          </div>
                        )
                      : elem.unreadSenderMessages > 0 && (
                          <div className="unread-count">
                            {elem.unreadSenderMessages}
                          </div>
                        )}
                  </div>
                ))
                ...
                (elem) =>
                  elem._id !== user._id && (
                    <div
                      key={elem._id}
                      className={`friend-item ${
                        this.isFriend(elem._id) ? 'friend' : ''
                      }`}
                      onClick={() => {
                        !this.isFriend(elem._id) && this.addChat(elem._id);
                      }}
                    >
                      <div className="friend-name">{elem.username}</div>

                      {elem.isOnline && <div className="online-mark"></div>}
                    </div>
                  ),
              )
            )}
