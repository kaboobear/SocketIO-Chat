...
class ChatInfo extends React.Component {
  render() {
    const { currentChat, user, isLoading, focused } = this.props;
            ...
            <div className="vert-center">
              <div className="chat-friend">
                {currentChat.sender._id === user._id
                  ? currentChat.talker.username
                  : currentChat.sender.username}
              </div>

              <div className="chat-friend-online">
                {currentChat.sender._id === user._id ? (
                  currentChat.talker.isOnline ? (
                    <div className="online-text">Online</div>
                  ) : (
                    <div className="time-age">
                      (online{' '}
                      <Moment fromNow>
                        {currentChat.talker.lastOnlineDate}
                      </Moment>
                      )
                    </div>
                  )
                ) : currentChat.sender.isOnline ? (
                  <div className="online-text">Online</div>
                ) : (
                  <div className="time-age">
                    (online{' '}
                    <Moment fromNow>{currentChat.sender.lastOnlineDate}</Moment>
                    )
                  </div>
                )}
              </div>
...
export default withRouter(
  connect(mapStateToProps, { deleteMessages })(ChatInfo),
);
